...
|---|-------|--------|---------|
| 1 | На 1440px заголовки всех 4 карточек Business Goals выровнены по нижнему краю карточки, как в Figma | ? HUMAN | `bottom-[120px]` применён в строке 287 business-goals.tsx, но значение выведено без Figma — визуальная сверка обязательна |
| 2 | На 1440px текстовый блок карточки #4 имеет ширину и font-weight, соответствующие Figma | ? HUMAN | `w-[390px]` и `font-normal` для index=3 реализованы (строки 287, 297), но без Figma-сверки значения не подтверждены |
...
| 5 | На 1180px ленты с логотипами клиентов растягиваются на всю ширину viewport без видимой обрезки | ? HUMAN | 100vw breakout применён корректно (`width: 100vw`, `marginLeft: calc(50% - 50vw)`), но отсутствие скроллбара требует браузерной проверки |
| 6 | Горизонтальный скроллбар не появляется после исправления LY1180-01 | ? HUMAN | `overflow-x-clip` убран, внешний div имеет `w-full min-w-0`, но финальный результат нельзя гарантировать статически |
...
|-------------|------------|-------------|--------|---------|
| LY1440-01 | 02-01-PLAN.md | Названия услуг выровнены по нижнему краю карточки как в Figma | ? HUMAN | Код реализован (`bottom-[120px]`), точность значения требует Figma-сверки |
| LY1440-02 | 02-01-PLAN.md | В карточке №4 длина текстового фрейма и начертание шрифта соответствуют Figma | ? HUMAN | Код реализован (`w-[390px]`, `font-normal`), точность требует Figma-сверки |
...
| LY1180-01 | 02-03-PLAN.md | Полосы прокрутки клиентов растянуты на полную ширину | ? HUMAN | 100vw breakout применён, но отсутствие скроллбара требует браузерной проверки |
