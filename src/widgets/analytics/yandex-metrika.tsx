...
function getYandexMetrikaSnippet(settings: SiteSettingsData): string {
...
export function YandexMetrikaHead({ settings }: YandexMetrikaProps) {
  if (!shouldRenderMetrika(settings)) return null;

  return (
    <script
      dangerouslySetInnerHTML={{ __html: getYandexMetrikaSnippet(settings) }}
      type="text/javascript"
    />
  );
}

export function YandexMetrikaNoScript({ settings }: YandexMetrikaProps) {
  if (!shouldRenderMetrika(settings)) return null;
